...
public class CommandLoader
{
    ...
    private static final List<CommandInfo> COMMAND_LIST;
    ...
    public static void scan()
    {
        ...
        if (commandMap == null)
        {
            Log.severe("[" + CleverMine.plugin.getName() + "] Error loading commands");
            return;
        }
        COMMAND_LIST.clear();
        COMMAND_LIST.addAll(getCommands());

        COMMAND_LIST.stream().map((commandInfo) -> new DynamicCommand(commandInfo)).forEach((dynamicCommand) -> {
            ...
            commandMap.register(CleverMine.plugin.getDescription().getName(), dynamicCommand);
        });

        Log.info("[" + CleverMine.plugin.getName() + "] All commands loaded");
    }
    ...
    {
        final Object commandMap = getField(Bukkit.getServer().getPluginManager(), "commandMap");
        if (commandMap != null)
        {
    ...
    private static List<CommandInfo> getCommands()
    {
        List<CommandInfo> commandList = new ArrayList<>();

        try
        {
            CodeSource codeSource = CleverMine.class.getProtectionDomain().getCodeSource();
            if (codeSource != null)
            {
    ...
    public static class CommandInfo
    {
        ...
        public CommandInfo(Class<?> commandClass, String commandName, SourceType source, boolean blockHostConsole, String description, String usage, String aliases)
        {
    ...
    public static class DynamicCommand extends Command implements PluginIdentifiableCommand
    {
        private final CommandInfo commandInfo;

        private DynamicCommand(CommandInfo commandInfo)
        {
